...
class MajorsBiWheelChartView: UIView {
    var chart: Chart!
...
private func drawTransitPlanetSymbols(context: CGContext) {
    ...
    let planetSignSymbols: [(planet: CelestialObject, imageName: String)] = [
        ...
        (.planet(.mercury), chartCake!.major.mercury.sign.keyName),
        (.planet(.venus), chartCake!.major.venus.sign.keyName),
        (.planet(.mars), chartCake!.major.mars.sign.keyName),
        (.planet(.jupiter), chartCake!.major.jupiter.sign.keyName),
        (.planet(.saturn), chartCake!.major.saturn.sign.keyName),
        (.planet(.uranus), chartCake!.major.uranus.sign.keyName),
        (.planet(.neptune), chartCake!.major.neptune.sign.keyName),
        (.planet(.pluto), chartCake!.major.pluto.sign.keyName),
        (.lunarNode(.meanSouthNode), chartCake!.major.southNode.sign.keyName)
        
    ]
    
    
    let planetMinute: [(planet: CelestialObject, minute: String)] = [
        (.planet(.sun), "\(Int(chartCake!.major.sun.minute))°"),
        (.planet(.moon), "\(Int(chartCake!.major.moon.minute))°"),
        (.planet(.mercury), "\(Int(chartCake!.major.mercury.minute))°"),
        (.planet(.venus), "\(Int(chartCake!.major.venus.minute))°"),
        (.planet(.mars), "\(Int(chartCake!.major.mars.minute))°"),
        (.planet(.jupiter), "\(Int(chartCake!.major.jupiter.minute))°"),
        (.planet(.saturn), "\(Int(chartCake!.major.saturn.minute))°"),
        (.planet(.uranus), "\(Int(chartCake!.major.uranus.minute))°"),
        (.planet(.neptune), "\(Int(chartCake!.major.neptune.minute))°"),
        (.planet(.pluto), "\(Int(chartCake!.major.pluto.minute))°"),
        (.lunarNode(.meanSouthNode), "\(Int(chartCake!.major.southNode.minute))°")
...
private func getTransitPlanets() -> [CGFloat] {
    let sunPosition = chartCake!.major.sun.value
    let moonPosition = chartCake!.major.moon.value
    let mercuryPosition = chartCake!.major.mercury.value
    let venusPosition = chartCake!.major.venus.value
    let marsPosition = chartCake!.major.mars.value
    let jupiterPosition = chartCake!.major.jupiter.value
    let saturnPosition = chartCake!.major.saturn.value
    let uranusPosition = chartCake!.major.uranus.value
    let neptunePosition = chartCake!.major.neptune.value
    let plutoPosition = chartCake!.major.pluto.value
//        let northNodePosition = chart!.northNode.value
    let southNodePosition = chartCake!.major.southNode.value
