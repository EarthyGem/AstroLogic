...
class CustomTableViewCellDeacnates: UITableViewCell {
...
//        planetGlyph.frame = CGRect(x: 10, y: 44, width: 12, height: 13)
        ...
        let planetTextHeight: CGFloat = 40
        let headerTextY = planetText.frame.origin.y - 20 + planetTextHeight

        let headerTextWidth: CGFloat = 225

        let textRect = headerText.textRect(forBounds: CGRect(x: 0, y: 0, width: headerTextWidth, height: .greatestFiniteMagnitude),
                                           limitedToNumberOfLines: 0)
        let headerTextHeight = ceil(textRect.height)

        headerText.frame = CGRect(x: 10, y: headerTextY + 15, width: headerTextWidth, height: headerTextHeight)
