...
class MinorProgressedAspectsViewController: UIViewController {
    ...
    override func viewDidLayoutSubviews() {
        ...
        func setTableContentSize(for celestialBody: String, on tableView: UITableView) {
            ...
            let aspectCount = chartCake.constructMinorAspectDictionary()[celestialBody]?.count ?? 0
...
extension MinorProgressedAspectsViewController: UITableViewDataSource, UITableViewDelegate {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {

        guard let planet = tableViewToPlanetMap[tableView],
              let majorAspects = chartCake?.constructMinorAspectDictionary()[planet.celestialObject.keyName] else {
            return 0
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let planet = tableViewToPlanetMap[tableView],
           let minorAspects = chartCake?.constructMinorAspectDictionary()[planet.celestialObject.keyName] {
            let aspect = minorAspects[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if let planet = tableViewToPlanetMap[tableView],
           let minorAspects = chartCake?.constructMinorAspectDictionary()[planet.celestialObject.keyName] {
            ...
            let selectedAspect = minorAspects[indexPath.row]
            navigateToTransitMatrixViewController(with: selectedAspect)
        }
    }
