...
class MinorsBiWheelChartView: UIView {
    var chart: Chart!
    var chartCake: ChartCake!
    var minorProgressedChart: ChartCake!
    
    var planetPositions: [CelestialObject: CGFloat] = [:]
    var minorPlanetPositions: [CelestialObject: CGFloat] = [:]
    var minorPlanetDegree: [(planet: CelestialObject, degree: String)] = []
...
private func drawTransitZodiacCircle(context: CGContext) {
    ...
    func getDegree(chartCake: ChartCake) -> [(planet: CelestialObject, degree: String)]  {
        let planetDegree: [(planet: CelestialObject, degree: String)] = [
            (.planet(.sun), "\(Int(chartCake.minor.sun.degree))°"),
            (.planet(.moon), "\(Int(chartCake.minor.moon.degree))°"),
            (.planet(.mercury), "\(Int(chartCake.minor.mercury.degree))°"),
            (.planet(.venus), "\(Int(chartCake.minor.venus.degree))°"),
            (.planet(.mars), "\(Int(chartCake.minor.mars.degree))°"),
            (.planet(.jupiter), "\(Int(chartCake.minor.jupiter.degree))°"),
            (.planet(.saturn), "\(Int(chartCake.minor.saturn.degree))°"),
            (.planet(.uranus), "\(Int(chartCake.minor.uranus.degree))°"),
            (.planet(.neptune), "\(Int(chartCake.minor.neptune.degree))°"),
            (.planet(.pluto), "\(Int(chartCake.minor.pluto.degree))°"),
            (.lunarNode(.meanSouthNode), "\(Int(chartCake.minor.southNode.degree))°")
...
private func drawTransitPlanetSymbols(context: CGContext) {
    ...
    let planetSignSymbols: [(planet: CelestialObject, imageName: String)] = [
        (.planet(.sun), chartCake!.minor.sun.sign.keyName),
        (.planet(.moon), chartCake!.minor.moon.sign.keyName),
        (.planet(.mercury), chartCake!.minor.mercury.sign.keyName),
        (.planet(.venus), chartCake!.minor.venus.sign.keyName),
        (.planet(.mars), chartCake!.minor.mars.sign.keyName),
        (.planet(.jupiter), chartCake!.minor.jupiter.sign.keyName),
        (.planet(.saturn), chartCake!.minor.saturn.sign.keyName),
        (.planet(.uranus), chartCake!.minor.uranus.sign.keyName),
        (.planet(.neptune), chartCake!.minor.neptune.sign.keyName),
        (.planet(.pluto), chartCake!.minor.pluto.sign.keyName),
        (.lunarNode(.meanSouthNode), chartCake!.minor.southNode.sign.keyName)
        
    ]
    
    
    let planetMinute: [(planet: CelestialObject, minute: String)] = [
        (.planet(.sun), "\(Int(chartCake!.minor.sun.minute))°"),
        (.planet(.moon), "\(Int(chartCake!.minor.moon.minute))°"),
        (.planet(.mercury), "\(Int(chartCake!.minor.mercury.minute))°"),
        (.planet(.venus), "\(Int(chartCake!.minor.venus.minute))°"),
        (.planet(.mars), "\(Int(chartCake!.minor.mars.minute))°"),
        (.planet(.jupiter), "\(Int(chartCake!.minor.jupiter.minute))°"),
        (.planet(.saturn), "\(Int(chartCake!.minor.saturn.minute))°"),
        (.planet(.uranus), "\(Int(chartCake!.minor.uranus.minute))°"),
        (.planet(.neptune), "\(Int(chartCake!.minor.neptune.minute))°"),
        (.planet(.pluto), "\(Int(chartCake!.minor.pluto.minute))°"),
        (.lunarNode(.meanSouthNode), "\(Int(chartCake!.minor.southNode.minute))°")
    
    ]
    
    
    let sortedPlanetSymbols = planetSymbols.sorted { (symbol1, symbol2) -> Bool in
        if let position1 = minorPlanetPositions[symbol1.planet], let position2 = minorPlanetPositions[symbol2.planet] {
            return position1 < position2
        }
        return false
    }
    ...
    let sortedPlanetDegree = planetDegree.sorted { (degree1, degree2) -> Bool in
        if let position1 = minorPlanetPositions[degree1.planet], let position2 = minorPlanetPositions[degree2.planet] {
            return position1 < position2
        }
        return false
    }
    ...
    let sortedPlanetSignSymbol = planetSignSymbols.sorted { (sign1, sign2) -> Bool in
        if let position1 = minorPlanetPositions[sign1.planet], let position2 = minorPlanetPositions[sign2.planet] {
            return position1 < position2
        }
        return false
    }
    ...
    let sortedPlanetMinute = planetMinute.sorted { (minute1, minute2) -> Bool in
        if let position1 = minorPlanetPositions[minute1.planet], let position2 = minorPlanetPositions[minute2.planet] {
            return position1 < position2
        }
        return false
    }
    ...
    for (_, (celestialObject, imageName)) in sortedPlanetSymbols.enumerated() {
        guard let position = minorPlanetPositions[celestialObject] else { continue }
        let angle = 2 * .pi - (position * .pi / 180) + .pi
        ...
        if let lastCenter = lastSymbolCenter, let lastPosition = lastCelestialObject.flatMap({ minorPlanetPositions[$0] }) {
            let distance = hypot(centerX - lastCenter.x, centerY - lastCenter.y)
...
//            northNodePosition,
    ...
    func updateTransitChart() {
        ...
        let planetPositions: [CelestialObject: CGFloat] = [
            .planet(.sun): chartCake.minor.rickysBodies[0].value - ascendantOffset,
            .planet(.moon): chartCake.minor.rickysBodies[1].value - ascendantOffset,
            .planet(.mercury): chartCake.minor.rickysBodies[2].value - ascendantOffset,
            .planet(.venus): chartCake.minor.rickysBodies[3].value - ascendantOffset,
            .planet(.mars): chartCake.minor.rickysBodies[4].value - ascendantOffset,
            .planet(.jupiter): chartCake.minor.rickysBodies[5].value - ascendantOffset,
            .planet(.saturn): chartCake.minor.rickysBodies[6].value - ascendantOffset,
            .planet(.uranus): chartCake.minor.rickysBodies[7].value - ascendantOffset,
            .planet(.neptune): chartCake.minor.rickysBodies[8].value - ascendantOffset,
            .planet(.pluto): chartCake.minor.rickysBodies[9].value - ascendantOffset,
            .lunarNode(.meanSouthNode): chartCake.minor.rickysBodies[10].value - ascendantOffset
        ]
    ...
    func updateMinorDegrees(_ newDegrees: [(planet: CelestialObject, degree: String)]) {
    ...
    func updateTransitPlanetPositions(newTransitPositions: [CelestialObject: CGFloat]) {
        minorPlanetPositions = newTransitPositions
        setNeedsDisplay()
    }
...
private func getTransitPlanets() -> [CGFloat] {
    let sunPosition = chartCake!.minor.sun.value
    let moonPosition = chartCake!.minor.moon.value
    let mercuryPosition = chartCake!.minor.mercury.value
    let venusPosition = chartCake!.minor.venus.value
    let marsPosition = chartCake!.minor.mars.value
    let jupiterPosition = chartCake!.minor.jupiter.value
    let saturnPosition = chartCake!.minor.saturn.value
    let uranusPosition = chartCake!.minor.uranus.value
    let neptunePosition = chartCake!.minor.neptune.value
    let plutoPosition = chartCake!.minor.pluto.value
//        let northNodePosition = chartCake!.natal.northNode.value
       let southNodePosition = chartCake!.natal.southNode.value
...
func minorsCalculateNonOverlappingPosition(celestialObject: CelestialObject, position: CGFloat, radius: CGFloat, lastSymbolCenter: inout CGPoint?, lastCelestialObject: inout CelestialObject?, minSymbolDistance: CGFloat) -> CGPoint {
    let angle = 2 * .pi - (position * .pi / 180) + .pi
    let center = CGPoint(x: bounds.midX, y: bounds.midY)
    var centerX = center.x + cos(angle) * radius
    var centerY = center.y + sin(angle) * radius
