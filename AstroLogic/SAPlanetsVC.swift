...
class SolarArcProgressionsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func setupTransitSigns() -> [String] {
        transitSigns = [
            chartCake?.icing.sun.sign.keyName,
            chartCake?.icing.moon.sign.keyName,
            chartCake?.icing.mercury.sign.keyName,
            chartCake?.icing.venus.sign.keyName,
            chartCake?.icing.mars.sign.keyName,
            chartCake?.icing.jupiter.sign.keyName,
            chartCake?.icing.saturn.sign.keyName,
            chartCake?.icing.uranus.sign.keyName,
            chartCake?.icing.neptune.sign.keyName,
            chartCake?.icing.pluto.sign.keyName,
            chartCake?.icing.southNode.sign.keyName,
            chartCake?.icing.ascendant.sign.keyName,
            chartCake?.icing.midheaven.sign.keyName
    ...
    func getTransitPositions(for date: Date) -> [String] {
       minorProgressedSigns = [
        chartCake?.icing.sun.formatted,
        chartCake?.icing.moon.formatted,
        chartCake?.icing.ascendant.formatted,
        chartCake?.icing.mercury.formatted,
        chartCake?.icing.venus.formatted,
        chartCake?.icing.mars.formatted,
        chartCake?.icing.jupiter.formatted,
        chartCake?.icing.saturn.formatted,
        chartCake?.icing.uranus.formatted,
        chartCake?.icing.neptune.formatted,
        chartCake?.icing.pluto.formatted,
        chartCake?.icing.southNode.formatted,
        chartCake?.icing.ascendant.formatted,
        chartCake?.icing.midheaven.formatted
        ].compactMap { $0 } // This will remove any nil values from the array
    ...
    override func viewDidLoad() {
        ...
        let solarArcBiWheelChartView = SolarArcBiWheelChartView(frame: CGRect(x: 0, y: 130, width: screenWidth, height: screenWidth), chartCake: chartCake!)
...
return planetDegree
    ...
    func updateChartCakeAndUI(for date: Date) -> ChartCake? {
        ...
        if let updatedChartCake = ChartCake(
            ...
            for body in updatedChartCake.icing.rickysBodies {
            ...
            for body in updatedChartCake.icing.rickysBodies {
                updatedTransitDegrees.append((planet: body.body, degree: "\(Int(body.degree))"))
                   ...
                   // Update the transit positions and degrees in your solarArcBiWheelChartView
                   solarArcBiWheelChartView.updateTransitPlanetPositions(newTransitPositions: updatedTransitPositions)
     //       solarArcBiWheelChartView.updateMinorDegrees(updatedTransitDegrees)

                   solarArcBiWheelChartView.setNeedsDisplay()
...
//    }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return (chartCake?.icing.planets.count)!
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.configure(signGlyphImageName: (chartCake.icing.planets[indexPath.row].body.keyName.lowercased()), planetImageImageName: (chartCake.icing.planets[indexPath.row].body.keyName.lowercased()), signTextText: (chartCake.icing.planets[indexPath.row].formatted), planetTextText: chartCake.icing.planets[indexPath.row].body.minorsName, headerTextText: "", capsuleText: chartCake.houseCusps.house(of: chartCake.icing.rickysBodies[safe: indexPath.row]!).houseString)
...
extension SolarArcProgressionsViewController: DatePickerDelegate {
