...
class SimpleNatalAspectsByHousesVC: UIViewController {
...
//      view.frame = CGRect(x: 0, y: 0, width: 400, height: 6000)
        ...
        for (index, tableView) in tableViews.enumerated() {
            let count = chartCake?.simplAspectsFilteredByHouseRulers(house: index + 1).count ?? 0
...
extension SimpleNatalAspectsByHousesVC: UITableViewDataSource, UITableViewDelegate {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        let tableViews = [firstTableView, secondTableView, thirdTableView, fourthTableView, fifthTableView, sixthTableView, seventhTableView, eighthTableView, ninthTableView, tenthTableView, eleventhTableView, twelfthTableView]

        if let index = tableViews.firstIndex(of: tableView) {
            return chartCake?.simplAspectsFilteredByHouseRulers(house: index + 1).count ?? 0
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let index = tableViews.firstIndex(of: tableView) {
            ...
            if let keyName = chartCake?.simplAspectsFilteredByHouseRulers(house: index + 1)[indexPath.row].basicAspectString {
