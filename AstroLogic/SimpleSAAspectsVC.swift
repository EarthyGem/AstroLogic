...
class SimpleSAAspectsPlanetsViewController  : UIViewController {
    ...
    override func viewDidLayoutSubviews() {
        ...
        sunTableView.contentSize.height = CGFloat(chartCake!.solarArcSimpleAspectsFiltered(by: Planet.sun.celestialObject).count * 90)
        //        sunTableView.contentSize = CGSize(width: view.frame.width, height: numbers)
        moonTableView.contentSize.height = CGFloat(chartCake!.solarArcSimpleAspectsFiltered(by: Planet.moon.celestialObject).count * 90)
        mercuryTableView.contentSize.height = CGFloat(chartCake!.solarArcSimpleAspectsFiltered(by: Planet.mercury.celestialObject).count * 90)
        venusTableView.contentSize.height = CGFloat(chartCake!.solarArcSimpleAspectsFiltered(by: Planet.venus.celestialObject).count * 90)
        marsTableView.contentSize.height = CGFloat(chartCake!.solarArcSimpleAspectsFiltered(by: Planet.mars.celestialObject).count * 90)
        jupiterTableView.contentSize.height = CGFloat(chartCake!.solarArcSimpleAspectsFiltered(by: Planet.jupiter.celestialObject).count * 90)
        saturnTableView.contentSize.height = CGFloat(chartCake!.solarArcSimpleAspectsFiltered(by: Planet.saturn.celestialObject).count * 90)
        uranusTableView.contentSize.height = CGFloat(chartCake!.solarArcSimpleAspectsFiltered(by: Planet.uranus.celestialObject).count * 90)
        neptuneTableView.contentSize.height = CGFloat(chartCake!.solarArcSimpleAspectsFiltered(by: Planet.neptune.celestialObject).count * 90)
        plutoTableView.contentSize.height = CGFloat(chartCake!.solarArcSimpleAspectsFiltered(by: Planet.pluto.celestialObject).count * 90)
    ...
    @objc func moreAspectsButtonTapped() {
        let flipSynastryVC = SAAspectsPlanetsViewController()
...
extension SimpleSAAspectsPlanetsViewController: UITableViewDataSource, UITableViewDelegate {
    // Create a mapping from each tableView to its associated Planet
    private var tableViewToPlanetMap: [UITableView: Planet] {
        return [
            sunTableView: .sun,
            moonTableView: .moon,
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        guard let planet = tableViewToPlanetMap[tableView] else { return 0 }
        return chartCake!.solarArcSimpleAspectsFiltered(by: planet.celestialObject).count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: NewAspectsCustomTableViewCell.identifier, for: indexPath) as? NewAspectsCustomTableViewCell,
              let planet = tableViewToPlanetMap[tableView] else {
            return UITableViewCell()
        }

        let aspectString = chartCake!.solarArcSimpleAspectsFiltered(by: planet.celestialObject)[indexPath.row].basicAspectString
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        // You can add your logic here if needed for didSelectRowAt
    }
}
