...
//          var aspects = chart!.allRickysNatalAspects
        ...
        geocoding(location: location!) { [self] (latitude, longitude) in
            ...
            getTimeZone(location: coordinate) { [self] timeZone in
               ...
               var sunAspects = chart!.aspects(for: chart!.sun, on: Date())
                ...
                print("parallelCusp Harmony?Discord: \(chart?.getCuspParallelHarmonyAndDiscordScoresByAspect())")
                
                print("parallel Harmony?Discord: \(chart?.getCelestialParallelHarmonyAndDiscordScoresByAspect())")
